...
from App_PADESCE.formations.models import AppelFormateur, Classe
    ...
    if classe.prestation:
        ...
        if beneficiaire_name:
            by_beneficiaire = AppelFormateur.objects.filter(
                is_active=True, beneficiaire__iexact=beneficiaire_name
            ).count()
        ...
        if filters:
            ...
            if samples:
                ...
                for sample in samples:
                    print(
                        f"     - {sample.reference_code}: prestataire='{sample.prestataire}', beneficiaire='{sample.beneficiaire}'"
                    )
