...
def fix_formateur_ids_directly():
    ...
    with connection.cursor() as cursor:
        ...
        cursor.execute("SELECT id, code, nom_complet FROM formations_formateur WHERE code = 'TEL698065452'")
        test_formateur = cursor.fetchone()
        if test_formateur:
            print(f"Formateur test: ID={test_formateur[0]}, Code={test_formateur[1]}, Nom={test_formateur[2]}")
