...
prestation = (
    Prestation.objects.filter(code__iexact="PRESTA064")
...
if not prestation:
    ...
    prestataire_name = prestation.prestataire.raison_sociale if prestation.prestataire else "None"
    print(f"   Prestataire: {prestataire_name}")
    beneficiaire_name = prestation.beneficiaire.nom_structure if prestation.beneficiaire else "None"
    ...
    if source_bundle:
        ...
        if matching_rows:
            ...
            for i, row in enumerate(matching_rows[:5], 1):
                ...
                print(
                    f"   {i}. {reference_code}: "
                    f"prestataire='{prestataire}', formation='{formation}'"
                )
        else:
