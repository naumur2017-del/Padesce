...
exact_match = AppelFormateur.objects.filter(is_active=True, prestataire__iexact=prestataire_name).count()
print(f"\n1. Exact match on prestataire='{prestataire_name}':")
print(f"   Records found: {exact_match}")

if exact_match > 0:
    samples = AppelFormateur.objects.filter(is_active=True, prestataire__iexact=prestataire_name)[:5]
    for sample in samples:
        print(f"   - {sample.reference_code}: beneficiaire='{sample.beneficiaire}', formation='{sample.formation}'")

# Search 2: Case-insensitive partial match
partial_match = AppelFormateur.objects.filter(is_active=True, prestataire__icontains="PONTAAH").count()
...
unique_prest = AppelFormateur.objects.filter(is_active=True).values_list('prestataire', flat=True).distinct().order_by('prestataire')[:20]
