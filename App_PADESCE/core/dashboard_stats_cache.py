    ...
    try:
        ...
        if stats["total_appels"] > stats["appels_en_attente"]:
            stats["taux_reussite"] = round(
                (stats["appels_reussis"] / (stats["total_appels"] - stats["appels_en_attente"]))
                * 100,
                2,
            )
        ...
        satisfaction_qs = SatisfactionApprenant.objects.filter(
            appel__in=completed_qs, q9_satisfaction_globale__isnull=False
        )

        satisfaction_values = satisfaction_qs.values_list("q9_satisfaction_globale", flat=True)
        if satisfaction_values:
            stats["satisfaction_moyenne"] = round(
                sum(satisfaction_values) / len(satisfaction_values), 1
            )
            stats["satisfaction_count"] = len(satisfaction_values)

        # Statistiques audio et formulaires
        stats["audio_count"] = (
            completed_qs.filter(audio_file__isnull=False).exclude(audio_file="").count()
        )

        # Compter les formulaires complets (approximation basée sur les réponses)
        stats["form_count"] = completed_qs.filter(answers__isnull=False).distinct().count()
        ...
        stats["top_prestations"] = list(
            completed_qs.values("prestataire").annotate(count=Count("id")).order_by("-count")[:10]
        )
...
def get_presence_summary_stats(filters: Dict = None) -> Dict:
    ...
    presence_stats = {"total_participants": 0}
    for control in CONTROL_KEYS:
        presence_stats[f"{control}_pr"] = 0
        presence_stats[f"{control}_ab"] = 0
        presence_stats[f"{control}_empty"] = 0
