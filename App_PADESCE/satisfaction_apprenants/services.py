...
def get_prestations_ranking(prestation_stats=None, order='desc'):
    ...
    prestation_regions = {p.code: p.beneficiaire.region for p in Prestation.objects.filter(actif=True).select_related('beneficiaire') if p.beneficiaire}
    ...
    for item in prestation_stats:
        ...
        region = prestation_regions.get(code.upper()) or beneficiary_map.get(item['beneficiaire'], "")
    ...
    ranking = sorted(ranking, key=lambda x: (x['score_global'], x['effectif']), reverse=(order == 'desc'))
    return ranking
