...
def _callable_phone_summary_from_appel_rows(appel_rows: list[dict]) -> dict[str, dict[str, int]]:
    ...
    for row in appel_rows:
        ...
        if not classe_label:
            continue
        phones = phone_variants(row.get("telephone1"), row.get("telephone2"))
        if not phones:
            continue
        callable_counts[classe_label] += 1
        for phone in phones:
            phone_counts[classe_label][phone] += 1

    summary: dict[str, dict[str, int]] = {}
    for classe_label in set(callable_counts) | set(phone_counts):
        duplicates = sum(count - 1 for count in phone_counts[classe_label].values() if count > 1)
        summary[classe_label] = {
            "callable_total": int(callable_counts.get(classe_label) or 0),
            "unique_phone_total": len(phone_counts[classe_label]),
...
def _build_appel_class_progress_snapshot(source_bundle: dict | None = None) -> dict:
    ...
    source_phone_summary_by_key: dict[str, dict[str, int]] = {}
    source_classes_by_key: dict[str, dict] = {}
    ...
    if source_bundle:
        for classe_label, count in count_callable_source_records_by_class(source_bundle).items():
            classe_key = normalize_network_lookup(classe_label)
            if classe_key:
                source_callable_counts_by_key[classe_key] = int(count or 0)
        for classe_label, summary in summarize_source_class_phone_coverage(source_bundle).items():
            classe_key = normalize_network_lookup(classe_label)
            if classe_key:
        ...
        for source_class in (source_bundle.get("classes") or {}).values():
            ...
            if not classe_key:
                continue
            source_classes_by_key[classe_key] = source_class
            if classe_id:
                label_by_key.setdefault(classe_key, classe_id)
                raw_labels_by_key[classe_key].add(classe_id)
    ...
    for classe_key in sorted(
        ...
        progress = {
            ...
            "target": target,
            "reached": reached,
            "pct": round((termines / callable_total) * 100, 1) if callable_total else 0,
            "remaining_to_target": remaining_to_target,
            "duplicate_phone_total": int(phone_summary.get("duplicate_phone_total") or 0),
            "unique_phone_total": int(phone_summary.get("unique_phone_total") or 0),
        }
        classe_progress.append(progress)
    ...
    if source_bundle:
        for prestation_key, class_keys in prestation_classes.items():
            ...
            prestation_finished = all(
                _source_class_is_finished(source_classes_by_key.get(classe_key, {}))
                for classe_key in class_keys
                if classe_key in source_classes_by_key
            )

            for classe_key in actionable_keys:
                progress = progress_by_key.get(classe_key, {})
                source_class = source_classes_by_key.get(classe_key, {})
                callable_total = int(progress.get("total") or 0)
                duplicate_phone_total = int(progress.get("duplicate_phone_total") or 0)
                ...
                if quick_finish and prestation_finished:
                    ...
                    priority_label = "Tres rapide"
                elif duplicate_phone_total == 0:
                ...
                recommended_classes.append(
                    {
                        ...
                        or "-",
                        "prestataire": source_class.get("prestataire")
                        or prestation_info.get("prestataire")
                        or "-",
                        "beneficiaire": source_class.get("beneficiaire")
                        or prestation_info.get("beneficiaire")
                        or "-",
                        "callable_total": callable_total,
                        "termines": int(progress.get("termines") or 0),
                        "target": int(progress.get("target") or 0),
                        "remaining_to_target": remaining_to_target,
                        "duplicate_phone_total": duplicate_phone_total,
                        "priority_label": priority_label,
                        "reason": reason,
                        "score": (
                            0 if prestation_finished else 1,
                            0 if quick_finish else 1,
                            remaining_count,
                            remaining_to_target,
                            0 if duplicate_phone_total == 0 else 1,
                            callable_total,
                            duplicate_phone_total,
                            (progress.get("classe") or "").casefold(),
                        ),
                    }
                )
...
def appels_index(request):
    ...
    return render(
        ...
        "appels/index.html",
        {
            ...
            "recommended_classes": optimization_snapshot["recommended_classes"],
