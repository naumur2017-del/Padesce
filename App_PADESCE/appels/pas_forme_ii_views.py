...
def index(request):
    ...
    rows = AppelPasFormeII.objects.filter(is_active=True).select_related("locked_by", "modified_by")
    ...
    return render(request, "appels/pas_forme_ii.html", {"rows": page_rows, "page_obj": page_obj, "pagination_tokens": build_pagination_tokens(page_obj), "querystring_no_page": params.urlencode(), "filters": filters, "thresholds": list(threshold_map.values()), "threshold_map": threshold_map, "campaign_segments": campaign_segments})
...
def save_form(request, pk):
    ...
    if row.formulaire_rempli_at and not (
        request.user.is_superuser or row.locked_by_id == request.user.id
    ):
        error = "Seul l'agent ayant réalisé l'appel ou un administrateur peut modifier ce formulaire."
        if wants_json:
