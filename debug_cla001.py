...
if not classe:
    ...
    if classe.prestation:
        ...
        if filters:
            combined = AppelFormateur.objects.filter(is_active=True, **{'__'.join([]): ''}).filter(filters).count()
            ...
            if samples:
                ...
                for sample in samples:
                    print(f"     - {sample.reference_code}: prestataire='{sample.prestataire}', beneficiaire='{sample.beneficiaire}', formation='{sample.formation}'")
        else:
