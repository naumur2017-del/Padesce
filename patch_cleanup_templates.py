...
home_content = re.sub(
    r'// Transcription Logic.*?// End Transcription Logic',
    '',
    home_content,
    flags=re.DOTALL
)
...
index_content = index_content.replace('<button type="button" class="btn-small js-transcription" {% if not a.audio_file %}disabled{% endif %}>Transcription</button>', '')

# Remove the transcription modal
index_content = re.sub(
    r'<div id="js-transcription-modal".*?</div>\s*</div>',
    '',
    index_content,
    flags=re.DOTALL
)
