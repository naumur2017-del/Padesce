...
content = content.replace('<div id="js-class-threshold-badge" style="display:inline-block; margin-left:5px;"></div>', '')
...
content = content.replace("badgeContainer.innerHTML = '<span class=\"badge status-badge badge-success\" style=\"font-size:0.85rem; padding:4px 8px;\">Objectif Atteint ✅</span>';", "")
content = content.replace("badgeContainer.innerHTML = `<span class=\"badge status-badge badge-warning\" style=\"font-size:0.85rem; padding:4px 8px;\">${prog.termines}/${prog.total} (${prog.pct}%)</span>`;", "")

# Also for the real-time v2 logic
content = content.replace("""            // Also update the standalone badge if this class is currently selected
...
content = content.replace("const badgeContainer = document.getElementById('js-class-threshold-badge');", "")
