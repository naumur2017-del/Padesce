...
def fix_formateur_ids():
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            "SELECT id, code, nom_complet FROM formations_formateur WHERE id IS NULL OR id = '' OR code = 'TEL698065452'"
        )
    ...
    try:
        ...
        if problematic_formateur and (
            not problematic_formateur.id or problematic_formateur.id == ""
        ):
    ...
    with connection.cursor() as cursor:
        ...
        if count > 0:
            ...
            cursor.execute(
                "SELECT MAX(id) FROM formations_formateur WHERE id IS NOT NULL AND id != ''"
            )
            max_id_result = cursor.fetchone()
            max_id = max_id_result[0] if max_id_result[0] else 0

            cursor.execute(
                "UPDATE formations_formateur SET id = id + 1 WHERE id IS NULL OR id = ''"
            )
