...
def main():
    ...
    try:
        ...
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='appels_appelformateur';"
        )
        ...
        if table_exists:
            ...
            if active_count > 0:
                cursor.execute(
                    "SELECT reference_code FROM appels_appelformateur WHERE is_active = 1 LIMIT 5;"
                )
