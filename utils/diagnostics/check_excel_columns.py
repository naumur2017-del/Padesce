...
def check_excel_columns():
    ...
    try:
        ...
        for i, col in enumerate(df.columns):
            if 'téléphone' in str(col).lower() or 'telephone' in str(col).lower() or 'tel' in str(col).lower():
                print(f"  {i+1:2d}. '{col}'")
        ...
        for i in range(min(5, len(df))):
            ...
            for col in df.columns:
                if 'formateur' in str(col).lower() or 'téléphone' in str(col).lower() or 'tel' in str(col).lower():
                    value = df.iloc[i][col]
                    print(f"  {col}: {value}")
