    ...
    try:
        ...
        for combo_key, combo_data in formateur_combinations.items():
            ...
            for code, prestation_info in prestation_officielles.items():
                if (
                    prestation_info["prestataire_nom"] == prestataire_nom.strip().lower()
                    and prestation_info["beneficiaire_nom"] == beneficiaire_nom.strip().lower()
                ):
            ...
            if code_prestation:
                ...
                q1_avg = (
                    round(sum(combo_data["q1_scores"]) / len(combo_data["q1_scores"]), 2)
                    if combo_data["q1_scores"]
                    else 0.0
                )
                q2_avg = (
                    round(sum(combo_data["q2_scores"]) / len(combo_data["q2_scores"]), 2)
                    if combo_data["q2_scores"]
                    else 0.0
                )
                q3_avg = (
                    round(sum(combo_data["q3_scores"]) / len(combo_data["q3_scores"]), 2)
                    if combo_data["q3_scores"]
                    else 0.0
                )
        ...
        for i, stat in enumerate(sorted_by_nb, 1):
            print(
                f"{i:2d}. {stat['label'][:50]:50s} | Nb: {stat['nb']:3d} | Q1: {stat['avgs'][0]:5.2f} | Q2: {stat['avgs'][1]:5.2f} | Q3: {stat['avgs'][2]:5.2f}"
            )
