...
def extraire_infos(df: pd.DataFrame, valeur: str) -> tuple[pd.DataFrame, list]:
    ...
    colonnes_cibles = [
        "statut", "prestation_id", "prestataire", "bénéficaire", "fénetre",
        "objectifs_padesce_t", "apprenants_inscrits_t", "formation",
        "taux__de_personnes_formées_nan", "taux__de_présence_moyen__nan",
        "taux_de_satisfaction_formateurs_nan", "taux_de_satisfaction_appreannts_nan",
    ...
    return subset[colonnes_existantes + ["taux_inscription_calcule"]], subset["prestation_id"].tolist()
...
def filtrer_classes(classe_df: pd.DataFrame, liste_prestations: list) -> pd.DataFrame:
    ...
    colonnes_cibles = ["Prestation ID", "Cohorte", "Nb personnes", "FORMATION", classe_df.columns[0]]
    colonnes_existantes = [c for c in colonnes_cibles if c in subset.columns]
