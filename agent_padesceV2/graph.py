...
from .config import add_to_memory, get_memory_context
from .orthographe import trouver_entite
from .pipelines import pipeline_code, pipeline_fiche
...
def node_router(state: AgentState) -> AgentState:
    ...
    response = _LLM_ROUTER.invoke(
        [
            SystemMessage(
                content=f"""Tu es un classificateur d'intention. Réponds UNIQUEMENT par un JSON valide.
...
def node_pipeline_fiche(state: AgentState) -> AgentState:
    ...
    if not valeur:
        valeur = (
            entites.get("prestataire")
            or entites.get("beneficiaire")
            or entites.get("prestation_id")
            or ""
        )
    ...
    if result.get("statut") in ["erreur", "non_trouve"]:
        ...
        if fallback_res.get("statut") not in ["erreur", "erreur_execution"]:
            result = {
                "statut": "non_trouve",
                "message": fallback_res.get(
                    "reponse", result.get("message", "Aucun résultat trouvé.")
                ),
                "detail": fallback_res,
            }
...
def node_format_response(state: AgentState) -> AgentState:
    ...
    if intent == "hors_cadre":
        ...
        final_response = {
            "intention": intent,
            "reponse": "🔍 Je n'ai pas trouvé de résultat correspondant à votre recherche. Vérifiez l'orthographe ou essayez un autre terme.",
            "details": result,
        }
    else:
        ...
        response = llm.invoke(
            [
                SystemMessage(
                    content="""Tu es un assistant professionnel et bienveillant. Tu reformules une réponse technique pour un utilisateur non-technicien.
...
def route_after_router(
    state: AgentState,
) -> Literal["pipeline_fiche", "pipeline_excel", "pipeline_question", "hors_cadre"]:
    return {
