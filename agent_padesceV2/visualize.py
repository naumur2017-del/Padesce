...
def visualize_graph(output_png: str = "graph_schema.png", show: bool = True) -> str:
    ...
    try:
        ...
        draw.text(
            (W // 2, 45),
            "Architecture avec sous-graphes",
            fill="#666666",
            anchor="mm",
            font=font_small,
        )
        ...
        for name, x, y, color in nodes:
            if name in ["START", "END"]:
                draw.ellipse(
                    [x - 25, y - 14, x + 25, y + 14], fill=color, outline="#2d3436", width=2
                )
            else:
        ...
        for label, y in steps_fiche:
            draw.rounded_rectangle(
                [35, y - 12, 245, y + 12], radius=5, fill="#d5f5e3", outline="#00b894"
            )
        ...
        for label, y in steps_excel:
            draw.rounded_rectangle(
                [295, y - 12, 495, y + 12], radius=5, fill="#d6eaf8", outline="#0984e3"
            )
        ...
        for label, y in steps_question:
            draw.rounded_rectangle(
                [545, y - 12, 745, y + 12], radius=5, fill="#fef9e7", outline="#f39c12"
            )
        ...
        for label, y in steps_hors:
            draw.rounded_rectangle(
                [795, y - 15, 965, y + 15], radius=5, fill="#fadbd8", outline="#e74c3c"
            )
        ...
        examples_data = [
            (
                "🟢 FICHE",
                ["Fiche de CEW", "Prestations de l'Extrême Nord", "Rapport du prestataire MINEFOP"],
                "#00b894",
                120,
            ),
            (
                "🔵 EXCEL",
                [
                    "Excel des prestataires de F1",
                    "Fichier avec colonnes X, Y",
                    "Export filtré statut TERMINÉ",
                ],
                "#0984e3",
                360,
            ),
            (
                "🟡 QUESTION",
                [
                    "Combien de prestataires ?",
                    "Répartition par statut",
                    "Total inscrits par région",
                ],
                "#f39c12",
                600,
            ),
        ...
        if decompte is not None:
            ...
            draw.text(
                (40, 750),
                f"decompte ({decompte.shape[0]} lignes) :",
                fill="#6c5ce7",
                anchor="lm",
                font=font,
            )
            draw.text(
                (40, 770), ", ".join(cols_dec) + "...", fill="#636e72", anchor="lm", font=font_small
            )
        ...
        if classe is not None:
            ...
            draw.text(
                (40, 800),
                f"classe ({classe.shape[0]} lignes) :",
                fill="#6c5ce7",
                anchor="lm",
                font=font,
            )
